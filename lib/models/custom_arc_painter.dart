...
class CustomArcPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    var starVal = 135.0 + start;

    canvas.drawArc(
        rect, radians(starVal), radians(270), false, backgroundPaint);

    Path path = Path();
    path.addArc(rect, radians(starVal), radians(end));
    canvas.drawPath(path, shadowPaint);

    canvas.drawArc(rect, radians(starVal), radians(end), false, activePaint);
  }
